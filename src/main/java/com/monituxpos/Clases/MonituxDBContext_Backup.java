...
public class MonituxDBContext_Backup {
    ...
    public static void init(DBProvider provider, String connectionString, String user, String password) {
        try {
            ...
            if (provider == DBProvider.H2 && !dbFolder.toFile().exists()) {
                dbFolder.toFile().mkdirs();
                System.out.println("📁 Carpeta de base de datos creada en: " + dbFolder.toAbsolutePath());
            }

            if (provider == DBProvider.H2 && !connectionString.contains("Resources/Database")) {
                System.out.println("⚠️ Advertencia: la ruta H2 no apunta a la carpeta portable esperada.");
            }
...
props.put("hibernate.use_sql_comments", "true");
            ...
            if (provider == DBProvider.H2) {
                File dbFile = dbFolder.resolve("H2-DB.mv.db").toFile();
                if (!dbFile.exists()) {
                    throw new IllegalStateException("❌ El archivo de base de datos no fue creado: " + dbFile.getAbsolutePath());
                }
            }
